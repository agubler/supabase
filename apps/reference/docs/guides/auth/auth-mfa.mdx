...
:::note
Multi-Factor Authentication is in early access preview only.
:::

Multi-factor authentication (MFA) or sometimes also called two-factor
authentication (2FA) is a way to add an additional layer of security to your
application by requiring users to verify their identity through one or more
additional verification steps.

It is considered a best practice for all but the simplest of applications.

Users who use weak passwords or have compromised social login accounts are
prone to malicious account takeovers. These can be prevented by using
multi-factor authentication because they require the user to provide proof of
both of these:
...
Supabase Auth implements only TOTP multi-factor authentication. This type of
multi-factor authentication uses a timed one-time password generated from an
authenticator app in the control of users.

You need to prepare your application for two important flows when adding MFA.
...
Adding multi-factor authentication to your app involves these three steps:
...
export function EnrollMFA({
  ...
  useEffect(() => {
    (async () => {
      ...
      setQR(`data:image/svg+xml;utf8,${data.totp.qr_code}`);
    })();
  }, []);
...
the user's current and possible authenticator assurance level (AAL).

Therefore if you receive a `currentLevel` which is `aal1` but a `possibleLevel`
...
- If the current level can be upgraded to the possible one, the MFA screen is
  shown.
...
```sql
json_query_path(auth.jwt(), '$.amr[$.amr.size()-1]')
```
...
- `$.amr[$.amr.size()-1]` is a SQL/JSON path expression that fetches the last
  entry of the `amr` claim.

Once you have extracted the last entry in the array, you can compare the
